...
Original file is located at
    https://colab.research.google.com/drive/1WicpBPXh0NjD7OxCyn7DujOca8n_vBem
...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        pred_grad = self.output * gradOutput
        self.gradInput = pred_grad - self.output * np.sum(pred_grad, axis=1, keepdims=True)
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = input.shape[0] * gradOutput - np.sum(gradOutput, axis=0, keepdims=True)
...
class LeakyReLU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        self.output = np.add(np.multiply(input, input > 0),
                             np.multiply(input * self.slope, (input < 0)))
        return self.output

    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.add(np.multiply(gradOutput, input > 0),
                                np.multiply(gradOutput * self.slope, input < 0))
        return self.gradInput
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        self.output = np.add(np.multiply(input, (input > 0)),
                             np.multiply((np.exp(input) - 1) * self.alpha, (input < 0)))
        return  self.output

    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.add(np.multiply(gradOutput, (input > 0)),
                                np.multiply(gradOutput,
                                            np.multiply(np.exp(input) * self.alpha, (input < 0))))
        return self.gradInput
...
class ClassNLLCriterionUnstable(Criterion):
    ...
    def updateOutput(self, input, target):
        ...
        # Your code goes here. ################################################
        self.output = -np.sum(target * np.log(input_clamp)) / input.shape[0] # input.shape[0] = batch_size
        return self.output
...
class ClassNLLCriterion(Criterion):
    ...
    def updateOutput(self, input, target):
        # Your code goes here. ################################################
        self.output = -np.sum(target*input)/input.shape[0] # input.shape[0] = batch_size
        return self.output

    def updateGradInput(self, input, target):
        # Your code goes here. ################################################
        self.gradInput = -target/input.shape[0] # input.shape[0] = batch_size
        return self.gradInput
...
class Conv2d(Module):
    ...
    def updateOutput(self, input):
        ...
        _, out_channels, kernel_h, kernel_w = self.W.shape
        ...
        for b in range(batch_size):
            for o in range(out_channels):
                for i in range(in_channels):
                    self.output[b, o] += scipy.signal.correlate(
                        padded_input[b, i],
                        self.W[o, i],
                        mode='valid'
                    )
    ...
    def updateGradInput(self, input, gradOutput):
        pad_size = self.kernel_size // 2
        # Zero-padding the gradOutput
        padded_gradOutput = skimage.util.pad(gradOutput, ((0, 0), (0, 0), (pad_size, pad_size), (pad_size, pad_size)), mode='constant')
        batch_size, in_channels, h, w = input.shape

        # Initialize gradInput
        self.gradInput = np.zeros_like(input)

        # Compute gradInput by correlating gradOutput with flipped kernels
        for b in range(batch_size):
            for i in range(in_channels):
                for o in range(self.out_channels):
                    flipped_kernel = np.flip(self.W[o, i], axis=(0, 1))
                    self.gradInput[b, i] += scipy.signal.correlate(
                        padded_gradOutput[b, o],
                        flipped_kernel,
                        mode='valid'
                    )
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        for o in range(self.out_channels):
            for i in range(self.in_channels):
                grad_W = np.zeros_like(self.W[o, i])
                for b in range(batch_size):
                    grad_W += scipy.signal.correlate(
                        padded_input[b, i],
                        gradOutput[b, o],
                        mode='valid'
                    )
                self.gradW[o, i] += grad_W
...
class MaxPool2d(Module):
    ...
    def updateOutput(self, input):
        ...
        batch_size, n_channels, h, w = input.shape
        k = self.kernel_size

        # Reshape the input tensor
        reshaped = input.reshape(batch_size, n_channels, h // k, k, w // k, k)
        # Permute dimensions to prepare for max pooling
        permuted = reshaped.transpose(0, 1, 2, 4, 3, 5)  # Bring k dimensions together
        # Compute max pooling
        self.output = permuted.max(axis=(4, 5))

        # Store max indices for backward pass
        self.max_indices = np.zeros_like(permuted, dtype=bool)
        max_vals = self.output[..., None, None]
        self.max_indices = (permuted == max_vals)
    ...
    def updateGradInput(self, input, gradOutput):
        batch_size, n_channels, h, w = input.shape
        k = self.kernel_size
        gradInput_shape = (batch_size, n_channels, h // k, k, w // k, k)

        # Initialize gradInput with zeros
        self.gradInput = np.zeros_like(input)

        # Expand gradOutput to match the shape of max_indices
        expanded_gradOutput = gradOutput[..., None, None]  # Add singleton dimensions
        gradInput_reshaped = np.zeros(gradInput_shape)
        gradInput_reshaped[self.max_indices] = expanded_gradOutput.ravel()

        # Reshape back to original input shape
        self.gradInput = gradInput_reshaped.transpose(0, 1, 2, 4, 3, 5).reshape(batch_size, n_channels, h, w)
